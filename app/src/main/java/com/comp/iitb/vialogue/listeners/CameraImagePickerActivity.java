...
/**
 * Created by jeffrey on 9/3/17.
 */


public class CameraImagePickerActivity implements View.OnClickListener {;
    private File mCameraFile;
    private Context mContext;
    private Activity mActivity;
    private Storage mStorage;

    public CameraImagePickerActivity(Storage storage, Context context,Activity activity) {
        mStorage = storage;
        mContext = context;
        mActivity = activity;
    }
...
//        mFragment.startActivityForResult(cameraIntent, SharedRuntimeContent.GET_CAMERA_IMAGE);
        ...
        Intent cameraIntent = new Intent(android.provider.MediaStore.ACTION_IMAGE_CAPTURE);
        cameraIntent.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(mCameraFile));
        mActivity.startActivityForResult(cameraIntent, SharedRuntimeContent.GET_CAMERA_IMAGE);
