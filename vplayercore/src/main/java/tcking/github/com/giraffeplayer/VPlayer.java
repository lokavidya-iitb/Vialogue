 ...
 */
public class VPlayer implements SimulationHandler {
    /**
    ...
    //Extra for new modification
    private long mPosition;
    private ArrayList<Long> mDurationList;
    private PlayerDialogAdapter mPlayerDialogAdapter;
    private int mCurrentPosition;
    private boolean mInvokeRunnable;
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            ...
            if (fromUser && isSimulation) {
                ...
                while (duration < newPosition) {
                    mediaIndex += 1;
                    duration += mDurationList.get(mediaIndex);
                }
    ...
    public VPlayer(final Activity activity) {
        ...
        try {
            ...
            Log.e(this.getClass().getName(), " loadLibraries error ", e);
        }
...
//                        Toaster.show("download rate:" + extra);
    ...
    public void play(List<PlayerModel> urlList) {
        initialize();
        isSimulation = true;
        $.id(R.id.app_video_loading).visible();
    ...
    public void addPlayerDialogAdapter(PlayerDialogAdapter playerDialogAdapter) {
        mPlayerDialogAdapter = playerDialogAdapter;
        mPlayerDialogAdapter.bind(this);
        mInvokeRunnable = true;
    ...
    private class SimulationAsyncTask extends AsyncTask<Void, Void, Void> {
        List<PlayerModel> urlList;

        public SimulationAsyncTask(List<PlayerModel> urlList) {
        ...
        protected Void doInBackground(Void... params) {
            for (PlayerModel url : urlList) {
                PlayerModel.MediaType type = url.getType();
                if (type != PlayerModel.MediaType.BROKEN
                        && type != PlayerModel.MediaType.IMAGE) {
                    long duration = 0;
                    if (url.getType() == PlayerModel.MediaType.VIDEO) {
                        duration = getVideoDuration(url.getPath());
                        playerModels.add(url);
                        mDurationList.add(duration);
                        mDuration += duration;
                    } else if (url.getType() == PlayerModel.MediaType.IMAGE_AUDIO) {
                        duration = getVideoDuration(url.getAudioPath());
                        playerModels.add(url);
                        mDurationList.add(duration);
                        mDuration += duration;
                    }
...
//        return timeInMilliSec;
    ...
    /**
     * using constants in VPlayer,eg: VPlayer.SCALETYPE_FITPARENT
     *
     * @param scaleType
     */
    ...
    public VPlayer forward(float percent) {
    ...
    public VPlayer onError(OnErrorListener onErrorListener) {
        this.onErrorListener = onErrorListener;
        return this;
    }

    public VPlayer onComplete(Runnable complete) {
        this.oncomplete = complete;
        return this;
    }

    public VPlayer onInfo(OnInfoListener onInfoListener) {
        this.onInfoListener = onInfoListener;
        return this;
    }

    public VPlayer onControlPanelVisibilityChang(OnControlPanelVisibilityChangeListener listener) {
     ...
     * @return
     */
    public VPlayer live(boolean isLive) {
        this.isLive = isLive;
        return this;
    }
    ...
    public VPlayer onControlPanelVisibilityChange(OnControlPanelVisibilityChangeListener listener) {
        this.onControlPanelVisibilityChangeListener = listener;
        return this;
    }
